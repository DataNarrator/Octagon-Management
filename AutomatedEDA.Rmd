...
```{r}
setwd("C:/Users/yphillips/OneDrive - morehouse school of medicine/Data/")
...
purchase_order <- convert_to_numeric(purchase_order, columns_to_convert)
...
# If you have missing variables not coded NA. Change the character (i.e ?, 999, .) to "NA" missing values in an entire dataset (dataframe).
Will skip this step as it is not applicable in this code
```{r}
#purchase_order[purchase_order == "-"] <- NA #ex. the dash,-, used for missing
```

# Determine which columns return column names containing missing observations. Only change the dataframe name (i.e. purchase_order) in the script below.
```{r}
...
# Explore but be cautious into dropping variables (i.e.removing data) until more is known about the data set
# Explore which variable(s) to drop;  variables with >50% of missing obs
